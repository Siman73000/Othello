...
/// Provide temporary, exclusive access to the global browser state without
/// creating long-lived `&mut` references to a `static mut` value. The state
/// value is moved out, passed to the closure as `&mut`, then moved back in.
fn with_state<F, R>(f: F) -> R
where
    F: FnOnce(&mut BrowserState) -> R,
{
    unsafe {
        let p: *mut Option<BrowserState> = ptr::addr_of_mut!(STATE);
        // Take the option out; if empty, initialize it.
        let mut owned = ptr::replace(p, None);
        if owned.is_none() {
            owned = Some(init_state());
        }
        let mut st = owned.unwrap();
        let res = f(&mut st);
        // Put it back.
        ptr::replace(p, Some(st));
        res
    }
}
...
fn navigate_current(push_hist: bool) {
    ...
    if push_hist {
        with_state(|st| push_history(st, &url));
    }
    ...
    match resp {
        Ok(resp) => {
            // Prepare the content outside the state closure then swap it in.
            let body = resp.body;
            let ct = resp
                .content_type
            ...
            let new_lines = if ct.contains("text/html") {
                ...
                web::layout::render_text_lines(&page.doc, &rules, cols)
            } else {
            ...
            with_state(|st| {
                st.status = format!("{} ({} bytes)", resp.status, body.len());
                st.lines = new_lines;
                st.scroll = 0;
            });
        }
...
fn render_content() {
    ...
    // Read the visible slice into a local vector to avoid holding a borrow into the static.
    let lines_slice = {
...
fn render_status() {
    ...
    let status_text = with_state(|st| st.status.clone());
    draw_text_nc(x0 + 12, sy + 4, &status_text, 0xBFBFBF, 0x1F1F1F);
}
...
pub fn handle_char(ch: u8, ctrl: bool) -> bool {
    ...
    match ch {
        ...
        // Enter
        13 | b'\n' => {
            let focus_url = with_state(|st| st.focus == Focus::UrlBar);
            if focus_url {
                // normalize caret to end
                with_state(|st| st.caret = st.url.len());
...
pub fn handle_wheel(delta: i32) -> bool {
    let empty = with_state(|st| st.lines.is_empty());
    if empty { return false; }

    // Positive delta typically means scroll up.
    if delta > 0 {
        let step = (delta as usize).saturating_mul(3);
        with_state(|st| st.scroll = st.scroll.saturating_sub(step));
    } else {
        let step = ((-delta) as usize).saturating_mul(3);
        with_state(|st| st.scroll = (st.scroll + step).min(st.lines.len().saturating_sub(1)));
    }
    true
}
